...
SELECT
	p.product_name Name,
	pl.product_line "Product Line",
	p.buy_price "Buy Price"
FROM
	products p,
	productlines pl
WHERE
...
SELECT
	c.contact_firstname "First Name",
	c.contact_lastname "Last Name",
	c.city City
FROM
	customers c
WHERE
...
FROM
	...
	products p
WHERE
	p.productline_id = pl.id
	and pl.product_line IN ('Vintage Cars', 'Classic Cars')
ORDER BY
	pl.product_line DESC, p.product_name; 
